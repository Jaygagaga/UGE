...
class SwiftMixin:
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        # Skip saving in checkpoint directories if callback is handling it (to avoid redundancy)
        use_graph_encoder = getattr(self.args, 'use_graph_encoder', False)
        is_checkpoint_dir = os.path.basename(output_dir).startswith('checkpoint-')
        should_save_tokenizer = (not is_adapter) or use_graph_encoder
        # Skip if it's a checkpoint directory and graph encoder is enabled (callback handles it)
        if should_save_tokenizer and not (is_checkpoint_dir and use_graph_encoder):
