...
class PtEngine(InferEngine):
    ...
    def _infer_forward(self, template: Template, inputs: Dict[str, Any], adapter_request: Optional[AdapterRequest],
        ...
        # These are processed in template._post_encode and injected into inputs_embeds
        inputs.pop('graphs', None)
        inputs.pop('has_graphs', None)
        inputs.pop('central_node_ids', None)
        # For embedding tasks, disable cache to avoid cache-related errors
        # Embedding tasks don't need caching and it can cause compatibility issues
        if self.model_info.task_type == 'embedding':
