...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def run(self):
        ...
        # Add OGD callback if OGD is enabled
        # NOTE: OGD trainer is created later in create_optimizer_and_scheduler(),
        # but we add the callback here. The callback will retrieve the trainer
        # from args._ogd_trainer or training_args._ogd_trainer when needed.
        callbacks = list(self.callbacks) if self.callbacks else []
        if getattr(args, 'use_ogd', False):
            from swift.plugin.ogd_callback import OGDCallback
            callbacks.append(OGDCallback())
            print("[OGD] Added OGD callback to training pipeline (OGD trainer will be created during optimizer initialization)", flush=True)
            logger.info("[OGD] Added OGD callback to training pipeline")
