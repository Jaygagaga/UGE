...
_pair_keys = ['messages', 'images', 'videos', 'audios', 'tools', 'objects', 'graphs']
...
class RowPreprocessor:
    ...
    @staticmethod
    def _validate_graph_files(row: Dict[str, Any]) -> bool:
        """
        Validate that all graph files in the row are valid and can be loaded.
        Checks graphs in 'graphs', 'positive_graphs', and 'negative_graphs' fields.
        Returns True if all graphs are valid (or no graphs present), False otherwise.
        """
        from swift.llm.template.vision_utils import validate_graph_file
        
        # Check main graphs field
        graphs = row.get('graphs', [])
        if graphs:
            for graph_path in graphs:
                if isinstance(graph_path, str):
                    if not validate_graph_file(graph_path):
                        logger.warning(f"Invalid graph file detected during preprocessing: {graph_path}. Sample will be skipped.")
                        return False

        
        # Check positive_graphs field (if present)
        positive_graphs = row.get('positive_graphs', [])
        if positive_graphs:
            # Handle nested lists (positive_messages can be a list of lists)
            if isinstance(positive_graphs[0], list):
                for graph_list in positive_graphs:
                    for graph_path in graph_list:
                        if isinstance(graph_path, str):
                            if not validate_graph_file(graph_path):
                                logger.warning(f"Invalid positive graph file detected: {graph_path}. Sample will be skipped.")
                                return False
            else:
                for graph_path in positive_graphs:
                    if isinstance(graph_path, str):
                        if not validate_graph_file(graph_path):
                            logger.warning(f"Invalid positive graph file detected: {graph_path}. Sample will be skipped.")
                            return False
        
        # Check negative_graphs field (if present)
        negative_graphs = row.get('negative_graphs', [])
        if negative_graphs:
            if isinstance(negative_graphs[0], list):
                for graph_list in negative_graphs:
                    for graph_path in graph_list:
                        if isinstance(graph_path, str):
                            if not validate_graph_file(graph_path):
                                logger.warning(f"Invalid negative graph file detected: {graph_path}. Sample will be skipped.")
                                return False
            else:
                for graph_path in negative_graphs:
                    if isinstance(graph_path, str):
                        if not validate_graph_file(graph_path):
                            logger.warning(f"Invalid negative graph file detected: {graph_path}. Sample will be skipped.")
                            return False
    ...
    def batched_preprocess(self, batched_row: Dict[str, Any], *, strict: bool,
        ...
        for row in rows:
            try:
                # Validate graph files BEFORE preprocessing (skip invalid samples early)
                if not self._validate_graph_files(row):
                    # Skip this sample - graph files are invalid
                    continue
    ...
    def _patch_arrow_writer():
        ...
        def _new_init(self, schema=None, features=None, *args, **kwargs):
            ...
            if features is not None:
                ...
                # Note: graphs may not use 'bytes' field, but keeping same structure for consistency
                features['graphs'] = [{'bytes': Value(dtype='binary'), 'path': Value(dtype='string')}]
                features['objects'] = {
    ...
    def __call__(
        ...
        *,
        ...
        # Ensure 'graphs' field is always a list of strings (even if empty)
        # This fixes type inference issues where empty lists might be inferred as null
        if 'graphs' in dataset.features:
            from datasets import Sequence, Value
            try:
                # Cast to ensure consistent type: list of strings
                graphs_feature = Sequence(feature=Value(dtype='string'), length=-1)
                dataset = dataset.cast_column('graphs', graphs_feature)
            except Exception as e:
                logger.warning(f"Could not cast 'graphs' column: {e}. This may cause type inference issues.")
