...
class StdTemplateInputs:
    ...
    videos: List[str] = field(default_factory=list)
    audios: List[str] = field(default_factory=list)
    graphs: List[Any] = field(default_factory=list)  # Graph file paths or graph objects
    objects: Dict[str, List[Any]] = field(default_factory=dict)
    ...
    def __post_init__(self):
        ...
        if self.graphs and not isinstance(self.graphs, (list, tuple)):
    ...
    def is_multimodal(self):
        return bool(self.images or self.audios or self.videos or self.graphs or self.objects)

    @classmethod
    def from_dict(cls, inputs: Dict[str, Any]) -> 'StdTemplateInputs':
        inputs = deepcopy(inputs)
        # Optional debug: inspect original graphs before any processing
        if os.environ.get('SWIFT_DEBUG_GRAPHS') == '1':
            try:
                logger.info(f"[graphs-debug] StdTemplateInputs.from_dict input keys={list(inputs.keys())}")
                logger.info(f"[graphs-debug] StdTemplateInputs.from_dict inputs.get('graphs')={inputs.get('graphs')}")
            except Exception:
                pass
        kwargs = {}
        ...
        # Explicitly handle graphs field to ensure it's always extracted from input dict
        # This is important because graphs may not be in structured message format
        graphs_data = inputs.get('graphs')
        if graphs_data is not None:
            if isinstance(graphs_data, str):
                graphs_data = [graphs_data]
            elif not isinstance(graphs_data, (list, tuple)):
                graphs_data = [graphs_data]
            # Override media_kwargs['graphs'] with actual data from input dict
            media_kwargs['graphs'] = list(graphs_data) if graphs_data else []
        # If graphs_data is None, keep media_kwargs['graphs'] as [] (from remove_messages_media)
        if os.environ.get('SWIFT_DEBUG_GRAPHS') == '1':
            try:
                logger.info(f"[graphs-debug] StdTemplateInputs.from_dict normalized graphs={media_kwargs.get('graphs')}")
            except Exception:
                pass
    ...
    def remove_messages_media(messages: Messages) -> Dict[str, Any]:
        res = {'images': [], 'audios': [], 'videos': [], 'graphs': []}
        for message in messages:
            ...
            for item in content:
                ...
                if key == 'text':
                    new_content += value
                    continue
                # image/audio/video/graph
                # image_url/audio_url/video_url/graph_url
