...
class Template(ProcessorMixin):
    ...
    def pre_forward_hook(self, model: nn.Module, args, kwargs):
        ...
        # Remove graph-related keys that shouldn't be passed to model.forward()
        # These are processed in _post_encode and injected into inputs_embeds
        kwargs.pop('graphs', None)
        kwargs.pop('has_graphs', None)
        kwargs.pop('central_node_ids', None)
    ...
    def register_post_encode_hook(self, models: List[nn.Module]) -> None:
        ...
        if self._handles:
            logger.info(
                "post_encode hooks already active (%d handles). Skipping registration.",
                len(self._handles),
            )
            return

        model_names = [model.__class__.__name__ for model in models]
        logger.info("Registering post_encode hooks on: %s", model_names)
        for model in models:
    ...
    def remove_post_encode_hook(self):
        ...
        if self._handles:
            logger.info(
                "Removed %d post_encode hook(s) from: %s",
                len(self._handles),
                [model.__class__.__name__ for model, _ in self._handles],
            )
        self._handles = []
    ...
    def print_inputs(self, inputs: Dict[str, Any]) -> None:
        ...
        # Extra debug: log media paths for images/graphs if present
        try:
            media_keys = []
            if self.task_type in {'embedding', 'reranker', 'generative_reranker'}:
                # Check both base and prefixed keys (anchor_/positive_/negative_)
                prefixes = ['anchor_', 'positive_', 'negative_'] if self.is_training else ['']
                for prefix in prefixes:
                    for media_name in ['images', 'graphs']:
                        media_key = f'{prefix}{media_name}'
                        if media_key in inputs and inputs.get(media_key) is not None:
                            media_keys.append(media_key)
            for mk in media_keys:
                logger.info(f'[{mk.upper()}] {inputs.get(mk)}')
        except Exception:
            pass
