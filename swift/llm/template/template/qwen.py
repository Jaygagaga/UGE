...
class Qwen2VLGraphTemplate(Qwen2VLTemplate):
    """Extended Qwen2VL template for multi-view training with graph modality"""
    graph_token_id = 151657
    placeholder_tokens = ['<|image_pad|>', '<|video_pad|>', '<|graph_pad|>']
    special_tokens = ['<image>', '<video>', '<audio>', '<graph>', '<bbox>', '<ref-object>', '<cot-process>', '<start-image>']
    ...
    def _pre_tokenize(self, context_list: List[Context], loss_scale_list: List[float],
        ...
        try:
            ...
            if os.environ.get('SWIFT_DEBUG_GRAPHS') == '1':
                ...
                _logger = get_logger()
                _logger.info(f"[graphs-debug] Qwen2VLGraphTemplate._pre_tokenize: has_graphs={bool(inputs.graphs)}, graphs_len={len(inputs.graphs) if inputs.graphs is not None else 0}")
                _logger.info(f"[graphs-debug] Qwen2VLGraphTemplate._pre_tokenize: contains '<graph>'={any(ctx == '<graph>' for ctx in context_list)}")
        except Exception:
            pass
        ...
        for context, loss_scale in zip(context_list, loss_scale_list):
            ...
            for token in split_tokens:
                ...
                if token == '<graph>' and inputs.is_multimodal and inputs.graph_idx < len(inputs.graphs):
                    try:
                        import os
                        if os.environ.get('SWIFT_DEBUG_GRAPHS') == '1':
                            from swift.utils import get_logger
                            _logger = get_logger()
                            _logger.info(f"[graphs-debug] Replacing <graph> token at graph_idx={inputs.graph_idx}")
                    except Exception:
                        pass
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.environ.get('SWIFT_DEBUG_GRAPHS') == '1':
                from swift.utils import get_logger
                _logger = get_logger()
                _logger.info(f"[graphs-debug] Qwen2VLGraphTemplate._encode: has_graphs={encoded.get('has_graphs')}, graphs_len={len(encoded.get('graphs', []))}")
        except Exception:
            pass
        return encoded
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if inputs is not None and 'graphs' in inputs and hasattr(model, 'graph_encoder'):
            ...
            if graphs:
                ...
                #     from swift.utils import get_logger
                #     _logger = get_logger()
                #     _logger.info(f"[TRACE] Qwen2VLGraphTemplate._post_encode: graphs detected, batch={len(graphs)}")
                # except Exception:
                #     pass
                # try:
                    with torch.autocast('cuda', enabled=True, dtype=torch.float16):
                        ...
                        #     from swift.utils import get_logger
                        #     _logger = get_logger()
                        #     _logger.info(f"[TRACE] Qwen2VLGraphTemplate._post_encode: graph_encoder returned tensor batch_shape={[tuple(t.shape) for t in batch_node_embeddings]}")
                        # except Exception:
                        #     pass
                        ...
                        # from swift.utils import get_logger
                        # _logger = get_logger()
                        # _logger.info(f"graph_encoder_spatial.py: batch_node_embeddings [:10]: {batch_node_embeddings[:10]}")
                    # try:
                    #     import os
                    #     if os.environ.get('SWIFT_DEBUG_GRAPHS') == '1':
                    #         from swift.utils import get_logger
                    #         _logger = get_logger()
                    #         _logger.info(f"[graphs-debug] Qwen2VLGraphTemplate._post_encode: graph_encoder returned batch={len(batch_node_embeddings)}")
                    # except Exception:
                    #     pass
                    ...
                    for graph_idx, node_embeddings in enumerate(batch_node_embeddings):
                        ...
                        for batch_idx in range(input_ids.size(0)):
                            ...
                            if graph_idx < len(graph_starts) and graph_idx < len(graph_ends):
                                ...
                                #     from swift.utils import get_logger
                                #     logger = get_logger()
                                #     logger.info(f"graph_encoder_spatial.py: start_idx: {start_idx}, end_idx: {end_idx}")
                                ...
                                if graph_pad_indices:
                                    ...
                                    try:
                                        ...
                                        if os.environ.get('SWIFT_DEBUG_GRAPHS') == '1':
                                            from swift.utils import get_logger
                                            _logger = get_logger()
                                            _logger.info(f"[graphs-debug] _post_encode: injected {len(graph_pad_indices)} node embeds for graph_idx={graph_idx} batch_idx={batch_idx}")
                                    except Exception:
                                        pass
        ...
        # from swift.utils import get_logger
        # logger = get_logger()
        # logger.info(f"[graphs-debug] Qwen2VLGraphTemplate._post_encode: inputs_embeds shape: {inputs_embeds.shape}")
        return result
...
# Register the graph template for multi-view training
register_template(QwenTemplateMeta(MLLMTemplateType.qwen2_vl_graph, template_cls=Qwen2VLGraphTemplate))
...
class Qwen2_5VLGraphTemplate(Qwen2VLGraphTemplate):
    """Extended Qwen2.5-VL template for multi-view training with graph modality"""
    version = 'v2_5'
    norm_bbox = 'none'


# Register the graph template for Qwen2.5-VL
register_template(QwenTemplateMeta(MLLMTemplateType.qwen2_5_vl_graph, template_cls=Qwen2_5VLGraphTemplate))
