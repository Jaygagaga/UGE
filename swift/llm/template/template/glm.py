...
class GLM4_1VGraphTemplate(GLM4_1VTemplate):
    """Extended GLM4.1V template for multi-view training with graph modality (non-MoE, good for embedding learning)"""
    placeholder_tokens = ['<|image|>', '<|video|>', '<|graph_pad|>']
    ...
    def _pre_tokenize(self, context_list: List[Context], loss_scale_list: List[float],
        ...
        # First call parent to handle images, videos
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        # Call parent to get vision embeddings
...
# Register the graph template for multi-view training (non-MoE, good for embedding learning)
register_template(GLM4_1VTemplateMeta(MLLMTemplateType.glm4_1v_graph, template_cls=GLM4_1VGraphTemplate))
