...
class ModelArguments:
    ...
    def _init_new_special_tokens(self):
        ...
        # Automatically add graph special tokens if use_graph_encoder is enabled
        # These tokens are used for graph modality in multi-view training
        if getattr(self, 'use_graph_encoder', False):
            graph_tokens = ['<|graph_pad|>', '<|graph_start|>', '<|graph_end|>']
            for graph_token in graph_tokens:
                if graph_token not in new_special_tokens:
                    new_special_tokens.append(graph_token)
            logger.info(f'Added graph special tokens for graph encoder: {graph_tokens}')
