...
class TunerArguments:
    ...
    # graph encoder for multi-view training
    use_graph_encoder: bool = False
    graph_num_layers: int = 2
    edge_dim: int = 64
    graph_max_nodes: Optional[int] = None  # Maximum nodes per graph (None = no limit). Reduces memory usage.
    use_spatial_encoding: bool = True
    spatial_embed_dim: int = 128
    spatial_frequency_num: int = 16
    use_edge_features: bool = True
    edge_use_distance: bool = True
    edge_use_direction: bool = True
    edge_use_displacement: bool = True
    use_gat: bool = True
    gat_heads: int = 4
    use_spatial_auxiliary: bool = False
    spatial_loss_weight: float = 0.1  # Weight for spatial auxiliary loss
    s2_level: int = 4  # S2 level for tokenization (4 → 3 tokens: 1 face + 2 grouped)
    s2_group_size: int = 2  # Group size for S2 tokens (2 → 3 tokens total)

    # OGD (Orthogonal Gradient Descent) for continual learning
    use_ogd: bool = False  # Enable OGD to preserve image-text alignment when adding graph modality
    ogd_memory_size: int = 1000  # Number of feature vectors (or gradients) to store per layer
    ogd_protected_layers: Optional[List[str]] = None  # List of layer names to protect (None = auto-detect)
    ogd_update_memory_batches: int = 100  # Number of batches to use for memory update
    ogd_use_ogd_plus: bool = False  # Use OGD+ (stores all previous tasks, not just one)
    ogd_use_gradients: bool = False  # If True, use gradient-based OGD (standard OGD+). If False, use feature-based (default, more memory-efficient)
