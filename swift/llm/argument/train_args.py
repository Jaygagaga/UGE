...
class TrainArguments(SwanlabArguments, TunerArguments, BaseArguments, Seq2SeqTrainingOverrideArguments):
    ...
    def __post_init__(self) -> None:
        ...
        if self.optimizer is None:
            ...
            elif getattr(self, 'use_ogd', False) and getattr(self, 'use_graph_encoder', False):
                # Auto-set optimizer to OGD-enabled multiview graph optimizer
                self.optimizer = 'ogd_multiview_graph'
        ...
        # Copy OGD-related attributes from TrainArguments to TrainingArguments
        # This ensures optimizer callbacks can access use_ogd and other OGD settings
        if hasattr(self, 'use_ogd'):
            self.training_args.use_ogd = self.use_ogd
        if hasattr(self, 'ogd_memory_size'):
            self.training_args.ogd_memory_size = self.ogd_memory_size
        if hasattr(self, 'ogd_protected_layers'):
            self.training_args.ogd_protected_layers = self.ogd_protected_layers
        if hasattr(self, 'ogd_update_memory_batches'):
            self.training_args.ogd_update_memory_batches = self.ogd_update_memory_batches
        if hasattr(self, 'ogd_use_ogd_plus'):
            self.training_args.ogd_use_ogd_plus = self.ogd_use_ogd_plus
        if hasattr(self, 'ogd_use_gradients'):
            self.training_args.ogd_use_gradients = self.ogd_use_gradients
    ...
    def _init_deepspeed(self):
        if self.deepspeed:
            ...
            # Ensure stage3_gather_16bit_weights_on_model_save is True for ZeRO-3 configs
            # This is critical for saving complete model checkpoints with ZeRO-3
            if 'zero_optimization' in self.deepspeed and self.deepspeed['zero_optimization'].get('stage') == 3:
                if 'stage3_gather_16bit_weights_on_model_save' not in self.deepspeed['zero_optimization']:
                    ...
                    logger.info('Set stage3_gather_16bit_weights_on_model_save=True for ZeRO-3 config')
                elif not self.deepspeed['zero_optimization'].get('stage3_gather_16bit_weights_on_model_save', False):
                    # Force it to True even if it was explicitly set to False
                    self.deepspeed['zero_optimization']['stage3_gather_16bit_weights_on_model_save'] = True
                    logger.warn('Overriding stage3_gather_16bit_weights_on_model_save to True for ZeRO-3 config')
